    ...
    return {
        ...
        source: `
            ...
            czm_material czm_getMaterial(czm_materialInput materialInput)
            {
                ...
                plainLayerW = pow(clamp(plainLayerW, 0.0, 1.0), 1.28);
                ...
                float plainGravel = clamp(0.62 * plainGravelCoarse + 0.38 * plainGravelFine, 0.0, 1.0);
                ...
                base *= (1.0 - 0.30 * cavity);
                float plainMask = plainLayerW;
                float plainBlend = 0.34 * clamp(plainBlendGain, 0.05, 1.3) * plainMask;
                base = mix(base, plainColor, plainBlend);
                base = mix(
                    base,
                    base * (0.84 + 0.36 * plainGravel),
                    0.44 * plainMask * clamp(rockDetailGain, 0.4, 1.8)
                );
                float plainGrain = fbm(uv * 182.0 + vec2(plainGravel * 2.6, terrainShape * 3.4));
                base = mix(
                    base,
                    base * (0.90 + 0.20 * plainGrain),
                    0.52 * plainLayerW * detailAliasingSuppress
                );
                ...
                float plainCrispMask = smoothstep(0.24, 0.92, plainLayerW)
                    * detailAliasingSuppress
                    * (1.0 - 0.35 * farW);
                float plainCrisp = (plainGrain * 2.0 - 1.0) * 0.28;
                finalColor = mix(
                    finalColor,
                ...
                finalColor = clamp(
                    finalColor + vec3(plainBi * 0.045 * plainCrispMask),
                    0.0,
                    1.0
                );
